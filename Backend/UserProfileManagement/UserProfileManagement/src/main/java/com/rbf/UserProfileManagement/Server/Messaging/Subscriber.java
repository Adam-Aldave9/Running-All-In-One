...
public class Subscriber {
    ...
    private void deleteSessionsFailed(Map<String, String> data) { //<user_id (owning partner), restored partner username>
        ...
        List<Object[]> feedback = partnersRepository.addPartner(data.get("user_id"), data.get("partner")); //rollback partner
        if(feedback.size() == 1) { //if partner was rolled back successfully
            ...
            feedback = partnersRepository.addPartner(data.get("user_id"), data.get("partner")); //rollback partner
    ...
    private void addPartner(Map<String, String> data) {
        ...
        List<Object[]> feedback = partnersRepository.addPartner(data.get("user_id"), data.get("partner")); //add partner
        if(feedback.size() == 1) { //if partner was added successfully
            ...
            feedback = partnersRepository.addPartner(data.get("user_id"), data.get("partner")); //add partner
            logger.info("Retry Result is: " + feedback.size());
            if(feedback.size() == 1) {
                logger.info("Partner added successfully in UPAM");
            }
